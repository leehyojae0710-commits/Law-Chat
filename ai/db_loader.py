...
DEFAULT_KEYWORDS: dict[LegalType, list[str]] = {
    ...
    "criminal": [
        ...
        # 2026-09 추가: "사기"만으로는 통화위조·문서위조 같은 "가해자가 직접 위조한" 판례
        # 위주로만 걸려서, "중고거래에서 가품/위조품을 모르고 산 피해자" 같은 소비자 관점
        # 질문에 맞는 판례/법령이 인덱스에 거의 없었다 (criminal_qa가 엉뚱한 통화위조 판례를
        # 참조해 답하는 문제의 원인 - RAG_DEBUG로 확인됨). 피해자/소비자 관점 자료를
        # 보강하기 위한 키워드.
        "중고거래 사기", "온라인 사기", "전자상거래 등에서의 소비자보호에 관한 법률",
        "상표법 위반", "부정경쟁방지 및 영업비밀보호에 관한 법률", "사기죄 피해자",
    ],
    "administrative": [
        # 행정법 총론
