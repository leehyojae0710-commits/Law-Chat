...
import { dismissNoticeForDays } from "../hooks/useActivePopupNotices";
...
export const NoticePopup = ({ notices, onCloseAll }: NoticePopupProps) => {
  ...
  // 공지가 바뀔 때마다 체크박스는 초기화하고, 10초 자동 넘김 타이머를 새로 건다.
  useEffect(() => {
  ...
  const handleClose = () => {
    if (hideForWeek && current) {
      dismissNoticeForDays(current.id, 7);
    }
    onCloseAll();
  };
      ...
      <div className="relative bg-white rounded-xl p-6 max-w-md w-full">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-semibold mb-2 whitespace-pre-line">{current.title}</p>
            ...
            {notices.map((n, i) => (
              <button
                ...
                className={`w-2 h-2 rounded-full transition-colors ${
                  i === index ? "bg-violet-600" : "bg-slate-200 hover:bg-slate-300"
                }`}
              />
            ))}
          </div>
