  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-8">
      <h1 className="mb-6 text-xl font-bold text-slate-900 dark:text-slate-100">
        즐겨찾기 <span className="ml-1 text-sm font-normal text-slate-400 dark:text-slate-500">총 {conversations.length}건</span>
      </h1>
      ...
      <div className="mb-6 flex gap-2">
        <button
          ...
          className={`rounded-lg px-4 py-2 text-sm font-medium ${
            tab === "consult" ? "bg-violet-600 text-white" : "border border-slate-200 text-slate-600 dark:border-slate-700 dark:text-slate-300"
          }`}
        >
          상담 즐겨찾기
        </button>
        <button
          ...
          className={`rounded-lg px-4 py-2 text-sm font-medium ${
            tab === "precedent" ? "bg-violet-600 text-white" : "border border-slate-200 text-slate-600 dark:border-slate-700 dark:text-slate-300"
          }`}
        >
          판례 즐겨찾기
        </button>
      ...
      {tab === "consult" ? (
        <>
          ...
          {!isLoading && conversations.length === 0 && (
            <p className="py-10 text-center text-sm text-slate-400 dark:text-slate-500">즐겨찾기한 상담이 없어요.</p>
          )}
          ...
          <div className="space-y-3">
            {conversations.map((c) => (
              <div
                key={c.id}
                className="flex items-center justify-between rounded-xl border border-slate-200 px-5 py-4 dark:border-slate-700"
              >
          ...
          <p className="mt-6 rounded-lg bg-slate-50 px-4 py-3 text-sm text-slate-500 dark:bg-slate-800 dark:text-slate-400">
        ...
        <div className="rounded-xl border border-dashed border-slate-200 px-5 py-10 text-center text-sm text-slate-500 dark:border-slate-700 dark:text-slate-400">
          ...
          <div className="mt-3">
            <Link to="/precedents" className="font-medium text-violet-600 hover:underline dark:text-violet-400">
              판례 검색으로 이동 →
            </Link>
          </div>
        </div>
